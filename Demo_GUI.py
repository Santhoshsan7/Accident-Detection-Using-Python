...
class PrettyWidget(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        btn = QtWidgets.QPushButton('ANOMALY DETECTION SYSTEM \n Predict Activity', self)

        Model_dir = 'G:/AnomalyDetectionCVPR2018/FinalCode/'
    ...
    def SingleBrowse(self):
        print("Select the Criminal Activity from the list....")
        print(" \n1.Abuse\n 2.Arrest\n 3.Arson\n 4.Assault\n 5.RoadAccidents\n 6.Robbery\n 7.Shooting\n")

        input_file="Shooting033_x264.mp4"
        #print(video_path)
        ...
        print(Total_frames)
        x = np.linspace(1, int(Total_frames), int(Total_frames))
        scores = Frames_Score
        ...
        while True:
            ...
            if flag:
                ...
                if jj==1:
                    ...
                    plt.draw()
                    print(sum(scores1[:i])/len(scores1[:i]))
                    chk=sum(scores1[:i])/len(scores1[:i])
                    if chk>0.1:
                        chflag=1
                        
                    #cv2.putText(frame,"Hello World!!!", (0,100), 0, 2, 255)
                    plt.pause(0.000000000000000000000001)

                pos_frame = cap.get(cv2.CAP_PROP_POS_FRAMES)
                print (str(pos_frame) + " frames")
                if chflag==1:
                    cv2.putText(frame,'Abnormal', (80, 30), font, 1, (0, 0, 255), 2, cv2.LINE_4)
